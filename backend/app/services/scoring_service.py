...
from app.models.models import ModelRegistry, Feature, ScoreRequest, DecisionRule
...
class ScoringService:
    ...
    def compute_score(self, party_id: int, model_version: str = None, 
        ...
        features = self._get_current_features(party_id, model.feature_list)
        
        # Step 4: Compute score based on model type
        if model.model_type == "scorecard":
            raw_score = self._compute_scorecard(features, model.model_config)
        elif model.model_type == "ml_model":
            raw_score = self._compute_ml_model(features, model.model_config)
        else:
    ...
    def _compute_ml_model(self, features: dict, model_config: dict) -> float:
        ...
        # Placeholder - will load model artifact and predict
        raise NotImplementedError("ML models not yet implemented")
